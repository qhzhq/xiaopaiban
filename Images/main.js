...
function findDimensions() {
    ...
    document.getElementById("window").style.top = "20px";
...
function jumpwindow(a) {
    scroll(0, 0);
    window.onresize = findDimensions;
    document.getElementById("background").style.display = "";
    document.getElementById("window").style.display = "";
    document.getElementById("engine").src = a;
}
function changewindowsize(a, b) {
    window_height = b;
    window_width = a;
    document.getElementById("engine").width = a - 12;
    document.getElementById("engine").height = b - 25;
    findDimensions();
    refIframe("loading");
}
function jumpwithoutwindow(a) {
    document.getElementById("engine").src = a;
}
function refIframe(a) {
    switch (a) {
      case "loading":
...
function run() {
    ...
    if (document.getElementById("autocopy") && document.getElementById("autocopy").checked) {
        // 排版后直接复制到剪贴板
        textbox.select();
        try {
            navigator.clipboard.writeText(textbox.value).then(function() {
                fm.showMsg('排版完成，已复制到剪贴板！', '');
            }).catch(function() {
                // 降级方案
...
function resize() {
    // 新布局下 textbox 使用 flex:1 自动撑满，无需手动计算高度
    // 保留此函数以兼容旧逻辑调用
    var textbox = document.getElementById("textbox");
    if (!textbox) return;
    // flex 布局已自动处理，清除任何手动设置的高度
